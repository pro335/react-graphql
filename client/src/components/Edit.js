...
class Edit extends Component {
  ...
  render() {
    ...
    return (
        <Query query={GET_BOOK} pollInterval={500} variables={{ bookId: this.props.match.params.id }}>
            {({ loading, error, data }) => {
                if (loading) return 'Loading...';
                if (error) return `Error! ${error.message}`;
        
                return (
                    <Mutation mutation={UPDATE_BOOK} key={data.book._id} onCompleted={() => this.props.history.push(`/list`)}>
                            ...
                            <div className="container">
                                <div className="panel panel-default">
                                    ...
                                    <div className="panel-body">
                                        <h4><Link to="/list" className="btn btn-primary">Book List</Link></h4>
                                        <form onSubmit={e => {
                                            e.preventDefault();
